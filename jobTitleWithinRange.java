//this class will take in the start date 
//and end date of the report and check if the input is valid before moving on to the next step of generating the report
//leave blank on end date will generate report for the month of the start date only
import java.util.*;
import java.text.SimpleDateFormat; 


public class jobTitleWithinRange {

    private int id = -1;
    private String name ="";

    public jobTitleWithinRange(int id){
        this.id = id;
    }

    public jobTitleWithinRange(String name){
        this.name = name;
    }


    public static void main(String[] args){//menu to ask for time period(job title id or name already taken care of in jobTitleSearch.java)

    }   
    //menu to ask for time period(job title id or name already taken care of in jobTitleSearch.java)
    ...
    while (true) {
        ...
        System.out.println("===== Job Title Report Menu =====");

        if(id != -1){
            System.out.println("Selected job title ID: " + id);
        } else {
            System.out.println("Selected job title Name: " + name);
        }
    ...
    public boolean checkValidDates(String startDate, String endDate, Scanner in) {
        ...
        try {
            ...
            if (start.after(end)) {
                ...
                if (id != -1){
                    jobTitlePrintTotal report = new jobTitlePrintTotal(id);
                    report.jobTitlePrint(sqlStart, sqlEnd);

                }
                else{
                    jobTitlePrintTotal report = new jobTitlePrintTotal(name);
                    report.jobTitlePrint(sqlStart, sqlEnd);
                }
